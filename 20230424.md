...
- 기본 값 매개변수
```Python
def mul(n1, n2=1): # 함수 호출 시 n2에 입력된 값이 없으면 기본 값 사용
    return n1 * n2
print(mul(1, 2))
print(mul(1))
...
def test_func2(x, test=None):
    ...
    print(x, test)
```
- 기본값이 있는 매개변수는 기본값이 없는 매개변수보다 뒤에 위치해야함, 아니면 Error 발생
```Python
def sub(n1, n3, n2=0):
    print(n1 - n2 - n3) 
sub(3, 2) # 1
